...
int hammer_batch_init()
{
	...
	batch->buf_A.keys = batch->buf_B.input_buf + config->batch_buf_size;
	batch->buf_A.pkt_offsets = batch->buf_B.keys + config->batch_job_max_num * AES_KEY_SIZE;
	batch->buf_A.ivs = batch->buf_B.pkt_offsets + config->batch_job_max_num * PKT_OFFSET_SIZE;
	...
	batch->buf_B.keys = batch->buf_B.input_buf + config->batch_buf_size;
	batch->buf_B.pkt_offsets = batch->buf_B.keys + config->batch_job_max_num * AES_KEY_SIZE;
	batch->buf_B.ivs = batch->buf_B.pkt_offsets + config->batch_job_max_num * PKT_OFFSET_SIZE;
...
int hammer_batch_job_add(hammer_batch_t *batch, hammer_connection_t *c, int length)
{
	...
	memcpy(batch->keys + AES_KEY_SIZE * job_num, c->key, AES_KEY_SIZE);
	batch->pkt_offsets[job_num] = batch->cur_buf->buf_length;
	memcpy(batch->ivs + AES_IV_SIZE * job_num, c->iv, AES_IV_SIZE);
